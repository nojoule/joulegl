        ...
        for buffer, location in self.targeted_overflowing_buffer_objects:
            for i in range(count):
                if self.current_buffer_id + i >= 0 and (
                    ...
                    buffer.bind_single(
                        (self.current_buffer_id + i) % len(buffer.handle), location + i
                    )
...
class LayeredVertexDataHandler(BaseDataHandler):
    ...
    def set(self, rendering: bool = False) -> None:
        self.layered_data_handler[self.current_layer_id][
            self.current_sub_buffer_id
        ].set(rendering)
    ...
    def __next__(self) -> VertexDataHandler:
        self.current_sub_buffer_id += 1
        if self.current_sub_buffer_id >= len(
            self.layered_data_handler[self.current_layer_id]
        ):
        ...
        return self.layered_data_handler[self.current_layer_id][
            self.current_sub_buffer_id
        ]
